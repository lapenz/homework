module QuestionsHelper
  ...
  def printWrongQuestion(userQuestion)
    if !userQuestion.description.blank?
      ...
      description += "<input value=\"#{userQuestion.question_id}\" type=\"hidden\" name=\"question[id]\">"
      description
    end
  end
  ...
  def replaceWithFields(description)
    ...
    description.sub!(originalAnswer, "<input class=\"answer-field\" type=\"text\" name=\"question[answers][]\" autocomplete=\"off\" maxlength=\"#{answerSize}\" style=\"width: #{((answerSize + 3) * 6.5)}px;\">")
  end
